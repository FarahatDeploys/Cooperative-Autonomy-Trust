...
void export_control() {
  ...
  class_<cr::VehicleControl>("VehicleControl")
      ...
      // Expose AugmentedRealityPath as a property
      .add_property("AugmentedRealityPath",
          +[](const cr::VehicleControl& self) {
              boost::python::list ar_path;
              for (const auto& point : self.augmented_reality_path) {
                  ar_path.append(boost::python::make_tuple(point.x, point.y, point.z));
              }
              return ar_path;
          },
          +[](cr::VehicleControl& self, const boost::python::list& py_list) {
              self.augmented_reality_path.clear();
              for (int i = 0; i < boost::python::len(py_list); ++i) {
                  boost::python::tuple point = boost::python::extract<boost::python::tuple>(py_list[i]);
                  self.augmented_reality_path.emplace_back(
                      boost::python::extract<float>(point[0]),
                      boost::python::extract<float>(point[1]),
                      boost::python::extract<float>(point[2]));
              }
          }
      )
